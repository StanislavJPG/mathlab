...
class MathQuiz(UUIDModelMixin, TimeStampedModelMixin, LifecycleModel):
    ...
    average_solve_time_statistic = models.DurationField(verbose_name=_('average solve time'), blank=True)

    max_time_to_solve = models.TextField(verbose_name=_('max time to solve'), blank=True)
    math_expressions_count = models.PositiveSmallIntegerField()  # denormilized field
...
class MathQuizScoreboard(UUIDModelMixin, TimeStampedModelMixin, LifecycleModel):
    solved_expressions = models.ManyToManyField('game_area.MathExpression')
    solved_quizzes = models.ManyToManyField('game_area.MathQuiz')
