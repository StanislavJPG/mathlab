function getRightCalculation(example, tofind, type) {
  ...
  var url = `/mathlab/solvexample/equations/?example=${encodedExample}&to-find=${tofind}&type=${type}`;
...
function getRightCalculationForMatrix(matrixA, matrixB, operator) {
  ...
  var url = `/mathlab/solvexample/matrix/?matrixA=${encodedMatrixA}&matrixB=${encodedMatrixB}&operator=${encodedOperator}`;
...
function getRightPercentsCalculation(example, percent, num, type) {
  var encodedExample = encodeURIComponent(example);
  var url = `/mathlab/solvexample/percents/?example=${example}&percent=${percent}&num=${num}&type=${type}`;
